...
\begin{frame}[t]{Hypermedia}
		...
		\scriptsize 
		User uses APIs based on his understanding with some form of communication support. Eventually, the user will need to follow on additional resources' calls. You might be thinking of:
		
		\scriptsize		
		\begin{itemize}
			\item<1-> How can the client/consumer know which request he needs to make next?
			\item<2-> How the request should look like?
			\item<3-> Which types of error should the client expect?
			
			\item<4->[] \normalsize The answer to this is \textit{"hypermedia"}
...
\begin{frame}[t, fragile]{Hypermedia}
		...
		\begin{itemize}
			...
			\item<4-> HATEOAS \\
				- In Rest, this is also called "Hypermedia as the Engine of Application State"
		\end{itemize}		
\end{frame}

\begin{frame}[t, fragile]{HATEOAS}
		\scriptsize		
		\begin{itemize}
			\item It allows us use the hypermedia links in the API response contents.
			\item It allows the client to dynamically navigate to resources by traversing the \underline{links}.
			\item [] 
				\begin{center}
   					\includegraphics[width=0.6\textwidth, height=0.5\textheight]{img/rest-maturity-model.png}
   				\end{center}
		\end{itemize}		

\vspace{8mm}
\tiny source: \href{https://martinfowler.com/articles/richardsonMaturityModel.html} {\textcolor{blue}{Richardson Maturity Model}} 	
\end{frame}
...
\begin{frame}[t, fragile]{HATEOAS}
		...
		\begin{itemize}
			\item 
				\begin{semiverbatim}
					\code{http://localhost:8080/greeting}	
				\end{semiverbatim}
   				\includegraphics[width=0.5\textwidth, height=0.3\textheight]{img/greeting-response.png}
   			\item<2->
				\begin{semiverbatim}
					\code{http://localhost:8080/greeting?name=User}	
				\end{semiverbatim}
   				\includegraphics[width=0.5\textwidth, height=0.3\textheight]{img/greeting-response-2.png}
